...
function findVideoLink(block) {
  ...
  return links.find((link) => isVideoLink(link)) || null;
}
...
export default function decorate(block) {
  ...
  if (textDiv) textDiv.classList.add('hero-home-text');

  // --- Auto-detect video from any <a> in the block ---
  const videoLink = findVideoLink(block);

  if (videoLink) {
    block.classList.add('hero-home-video');
    ...
    video.className = 'hero-home-video-bg';
  ...
  if (badgeText && textDiv) {
    ...
    badgeEl.className = 'hero-home__badge';
  ...
  //     badge (a lone badge should still render above an otherwise-empty hero-home) ---
